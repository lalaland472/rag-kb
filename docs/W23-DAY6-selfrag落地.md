...
## 七、hybrid 回归测试：rerank 在 hybrid 模式是负优化

### 核心指标
| 指标 | 无 rerank 基线 | 带 rerank | 变化 |
|------|:---:|:---:|:---:|
| 完全命中+有内容 | 15/20 | **13/20** | -13% |
| 平均耗时 | 8.0s | 14.9s | +86% |

### 逐条
- C4：0%→50% ↑（唯一明显改善，补上 HyDE）
- B4：33%→0% ↓（中文口语跨篇，又被带偏）
- D3 保持 0%（基线就查不到）
- 其余 16 条持平

### 关键洞察
**rerank 在 hybrid 模式基本是负优化**：
1. hybrid 强项在「摘要节点加权」（RAPTOR 树把分散全文浓缩成主题节点），已做语义聚焦
2. 加 rerank 白花 ~7s，还把摘要命中的 doc 级结果重排成叶子级，破坏跨篇主题融合
3. B4 类口语 query 在 rerank 下更容易被单一侧面带偏

### 结论：rerank 需按模式路由（与 Self-RAG on-demand 一致）
| 模式 | rerank | 原因 |
|------|:---:|------|
| **flat** | ✅ 开（默认）| +20%，修引用精准度 |
| **hybrid** | ⚠️ 关（默认）| -13%，破坏摘要融合 |

**当前 `rerank=mode != "hybrid"` 的默认设置正确**：flat 开、hybrid 关，正好规避 hybrid 负优化。
